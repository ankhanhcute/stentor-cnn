...
print(f"Building cache for {len(HOLDOUT)} holdout datasets...")
for i, name in enumerate(HOLDOUT):
    ...
    if os.path.exists(processed_cache):
        print(f"  [{i+1}/{len(HOLDOUT)}] already cached: {name}")
        continue
    print(f"  [{i+1}/{len(HOLDOUT)}] processing: {name}")
...
print("All holdout caches built!")
