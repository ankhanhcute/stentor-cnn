...
def main():
    ...
    processed_data = np.load(args.processed_npy, mmap_mode='r')
    
    with h5py.File(args.meta_h5, 'r') as m:
        crop_size = int(m['crop_size'][()])
        num_cells = int(m['num_cells'][()])
     # load raw tiles for bg stats + artifact mask per cell
    ...
    raw = np.transpose(raw, (2, 3, 1, 0)).astype(np.float32) / 255.0
